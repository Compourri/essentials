...
Mullvad profiles require DNS over HTTPS support in Windows. If the selected primary resolver is unavailable, Essentials uses the closest Mullvad secondary resolver to preserve connectivity; that fallback may use a different filtering level and can be less restrictive.
