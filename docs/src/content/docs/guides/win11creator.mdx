---
title: Win11 Creator
description: Build a custom, debloated Windows 11 ISO from an official Microsoft image using Essentials's Win11 Creator tool.
sidebar:
  order: 7
---
...
## Using Essentials's Win11 Creator
...
![Win11 Creator tab in Essentials](../../../assets/screenshots/win11creator-tab-new.png)
...
- If enabled, Essentials exports the drivers from your current system, stages boot-storage drivers for Windows Setup, and injects eligible packages into the selected `install.wim` image. Extension-class packages and stale duplicates are excluded. Each remaining package is added separately, so one incompatible package does not stop the others. If a package fails, Essentials discards the partial mount and retries the remaining packages against the original image. Check the live log for warnings: if every package fails, Essentials keeps the original `install.wim` and still produces the ISO.
...
:::note
The resulting ISO is close to the size of the source ISO. Essentials does not remove the unused editions from `install.wim`; it selects your edition through `sources\ei.cfg` and `autounattend.xml` so Windows Setup installs the right one.
:::
...
:::note
Newer Windows 11 ISOs may not boot correctly on older versions of Ventoy — make sure Ventoy is up to date before use. If issues persist after updating, this is a Ventoy compatibility limitation outside of Essentials's control.
:::
