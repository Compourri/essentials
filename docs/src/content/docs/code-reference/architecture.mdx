...
description: How Essentials's GUI, functions, and JSON configuration files fit together, plus a deep dive into the Win11 Creator subsystem.
---
...
**Naming Convention**: Functions start with `WPF` or `Essentials` to be loaded into the runspace.
...
The **Win11 Creator** is a specialized subsystem within Essentials that creates customized Windows 11 ISOs. It operates independently of the main package installation and tweak system.
...
Essentials runs on three kinds of thread, and each has one job:
...
`$sync`, the compiled script's globals, and every Essentials function. That is what lets any
...
At startup, Essentials loads all configurations:
...
2. File naming must include "WPF" or "Essentials" to load
...
- Must include "WPF" or "Essentials" to load
