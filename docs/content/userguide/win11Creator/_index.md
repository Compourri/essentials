...
1. Open Software Essentials and go to the **Win11 Creator** tab.
...
2. Software Essentials mounts the ISO, checks for a valid `install.wim` or `install.esd`, and reads the available editions (Home, Pro, Enterprise, etc.).
...
Click **Run Windows ISO Modification and Creator** to start the customization process. Software Essentials will:
...
A live log shows progress as each step completes. This stage usually takes **10–30 minutes** depending on disk speed. The WIM dismount near the end is the slowest part, so do not close Software Essentials while it is running.
...
{{< tabs >}}
  ...
  3. Software Essentials builds a dual BIOS/UEFI bootable ISO using `oscdimg.exe`.

  > [!NOTE]
  > `oscdimg.exe` (part of the Windows ADK) is required. If it's not found, Software Essentials will attempt to install it automatically via WinGet. If that fails, install it manually: `winget install -e --id Microsoft.OSCDIMG`
  ...
  4. Software Essentials formats the drive as GPT with a 512 MB EFI partition and copies the modified Windows files.
...
| "Access Denied" error | Make sure Software Essentials is running as Administrator |
| "Setup has failed to validate the product key" | Recreate the ISO with the latest Software Essentials. The creator now removes stale `PID.txt`, writes `sources\ei.cfg`, and pins the selected image in `autounattend.xml` so setup does not use an embedded OEM key for a different edition |
...
> [!NOTE]
> Newer Windows 11 ISOs may not boot correctly on older versions of Ventoy — make sure Ventoy is up to date before use. If issues persist after updating, this is a Ventoy compatibility limitation outside of Software Essentials' control.
