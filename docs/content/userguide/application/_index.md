...
Use the Applications tab to install, upgrade, uninstall, and review supported apps from one place. Software Essentials relies on package manager support for these actions, so the available results depend on what WinGet can detect and manage on your system.
