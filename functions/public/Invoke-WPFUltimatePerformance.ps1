function Invoke-WPFUltimatePerformance {
    ...
    if ($Do) {
        if (-not (powercfg /list | Select-String "Ultimate Power Plan")) {
            ...
            powercfg /changename $guid "Ultimate Power Plan"
            ...
            Write-Host "Ultimate Power Plan plan is already installed." -ForegroundColor Red
            return
        }
    } else {
        if (powercfg /list | Select-String "Ultimate Power Plan") {
            powercfg /setactive SCHEME_BALANCED
            powercfg /delete ((powercfg /list | Select-String "Ultimate Power Plan").ToString().Split()[3])
            Write-Host "Ultimate Power Plan plan was removed." -ForegroundColor Red
        } else {
            Write-Host "Ultimate Power Plan plan is not installed." -ForegroundColor Yellow
        }
    }
}
